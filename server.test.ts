...
describe("Garden of Zo catalogue", () => {
  test("catalogues all public and private router apps", () => {
    ...
    expect(html).toContain('aria-label="Filter list by realm type"');
    ...
    expect((html.match(/data-kind="app"/g) ?? []).length).toBe(6);
    expect((html.match(/data-kind="workflow"/g) ?? []).length).toBe(1);
    expect((html.match(/data-kind="agent"/g) ?? []).length).toBe(1);
